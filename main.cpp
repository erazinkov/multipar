...
//        std::regex m{R"((sample(?:4[8-9]|5[0-3]|7[6-9]|8[0-5])\.sub))"}; // data_chem_cat_4_check
        ...
        auto value{ChemResult::Type::W};
        auto xTitle{value == ChemResult::Type::A ? ";A_{m}', %;A_{c}, %" : ";W_{m}', %;W_{c}, %"};
        if (value == ChemResult::Type::A) {
            points_p.insert(points_p.end(), points_p_a.cbegin(), points_p_a.cend());
        } else if (value == ChemResult::Type::W) {
        ...
        ss.str("");ss.clear();
        ss << "stdAbs=" << doubleToString(calculateStdAbsCon(points_p)) << xTitle;
         ...
         for (size_t i{0}; i < points_p.size(); ++i) {
             excludedPoints.push_back(ExcludedPoint{points_p.at(i), std::abs(points_p.at(i).x - points_p.at(i).y)});
                 ...
                 excludedPoints.erase(std::remove_if(excludedPoints.begin(), excludedPoints.end(),
                                              [](const ExcludedPoint& a) { return a.d <= 1.0; }),
                               excludedPoints.end());
